...
class PublicationTask {
  ...
  */
  ...
  async publish() {
    const content = await getPressReleaseContent(this.graph, this.pressRelease);

    const htmlContent = await this.createPressReleaseHtmlContent(content);

    await savePressReleaseText(this.graph, this.uri, htmlContent);
    await generatePressReleaseXmlFile(htmlContent, content[0].title, this.publicationDate);

    //TODO await sendToBelga();
  }

  async createPressReleaseHtmlContent(content) {
    const sources = await getPressReleaseSources(this.graph, this.pressRelease);

    return generateHtmlContent(content, sources);
  }
